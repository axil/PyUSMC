...
class _SettingsBase(Structure):
    ...
    def Set(self, **kwargs):
        """Helper function to set parameters. Same functionality could be
...
class USMC_Parameters(_SettingsBase):
    ...
    def Refresh(self):
        errCode = self._dll.USMC_GetParameters(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)

    def Apply(self):
        errCode = self._dll.USMC_SetParameters(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)
...
class USMC_StartParameters(_SettingsBase):
    ...
    def Refresh(self):
        errCode = self._dll.USMC_GetStartParameters(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)
...
class USMC_Mode(_SettingsBase):
    ...
    def Refresh(self):
        errCode = self._dll.USMC_GetMode(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)

    def Apply(self):
        errCode = self._dll.USMC_SetMode(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)
...
class USMC_State(_SettingsBase):
    ...
    def Refresh(self):
        errCode = self._dll.USMC_GetState(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)
...
class USMC_EncoderState(_SettingsBase):
    ...
    def Refresh(self):
        errCode = self._dll.USMC_GetEncoderState(self._motor.index, pointer(self))
        self.ProcessErrorCode(errCode)
...
class StepperMotorController:
    ...
    def Init(self):
        ...
        errCode = self._dll.USMC_Init(pointer(self._devices))
        print('number of stepper devices =', self._devices.NOD)
        self.ProcessErrorCode(errCode)
...
class Motor:
    ...
    def Start(self, destPos, speed = None):
        ...
        errCode = self._dll.USMC_Start(self.index, self.position.ToUSMCPos(destPos), \
            speed, pointer(self.startParameters))
        self._controller.ProcessErrorCode(errCode)
